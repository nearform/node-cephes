...
export default class LineGraph {
  constructor({ container, height, colors = schemePaired }) {
    ...
    this._xScale = scaleLinear().domain([1, 0]);
    this._yScale = scaleLinear().domain([1, 0]).range([this._innerHeight, 0]);

    // create grid
    this._xGrid = axisBottom(this._xScale).ticks(8).tickSize(-this._height);
    this._xGridElement = this._graph
      .append("g")
      .attr("class", "grid")
      .attr("transform", "translate(0," + this._innerHeight + ")");

    // create grid
    this._yGrid = axisLeft(this._yScale).ticks(8);
    this._yGridElement = this._graph.append("g").attr("class", "grid");

    // define axis
    this._xAxis = axisBottom(this._xScale).ticks(4);
    this._xAxisElement = this._graph
      .append("g")
      .attr("class", "axis")
      .attr("transform", "translate(0," + this._innerHeight + ")");

    this._yAxis = axisLeft(this._yScale).ticks(4);
  ...
  _drawLegend() {
    const legendSelectPhase1 = this._legend
      .selectAll("svg.legend-item")
    ...
    const legendSelectPhase2 = this._legend
      .selectAll("svg.legend-item")
  ...
  resize() {
    ...
    // draw lines
    const curveSelect = this._lines
      .selectAll("path.mathline")
      .data(this._data.lines);
