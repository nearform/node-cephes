    ...
    const compiled = async function () {
      const entries = await Promise.all(
        Object.entries(WASM_CODE).map(([pkg, code]) =>
          WebAssembly.instantiate(code, thisCephes.getWasmImports(pkg)).then(
...
const create = (real, imag) => new ComplexNumber(real ?? 0, imag ?? 0);
